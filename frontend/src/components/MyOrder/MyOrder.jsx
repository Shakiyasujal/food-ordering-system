...
const MyOrder = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const response = await axios.get('https://food-ordering-system-backend-qlrh.onrender.com/api/orders', {
                                                    ...
                                                    <div className='space-y-2'>
                                                        {order.items.map((item, index) => (
                                                            <div key={index} className='flex items-center gap-3 p-2 bg-[#3a2b2b]/50 rounded-lg'>
                                                                <img src={`https://food-ordering-system-backend-qlrh.onrender.com${item.item.imageUrl}`} alt={item.item.name} className='w-10 h-10 object-cover rounded-lg' />
