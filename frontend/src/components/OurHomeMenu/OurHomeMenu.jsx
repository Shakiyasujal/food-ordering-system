...
const OurHomeMenu = () => {
    ...
    useEffect(() => {
        axios.get('https://food-ordering-system-backend-0o7i.onrender.com/api/items').then(res => {
        ...
        px-4 sm:px-6 lg:px-8'>
            <div className='max-w-7xl mx-auto'>
                ...
                <div className='flex flex-wrap justify-center gap-4 mb-16'>
                    {categories.map(cat => (
                        <button key={cat} onClick={() => setActiveCategory(cat)}
                            className={`px-4 sm:px-6 py-2 rounded-full border-2 transition-all duration-300 transform font-conzel text-sm
                            sm:text-lg tracking-widest background-blur-sm
                            ${activeCategory === cat ?
                        ...
                        return (
                            ...
                            backdrop-blur-sm flex flex-col transition-all duration-500'
                                ...
                                <div className='p-4 sm:p-6 flex flex-col flex-grow'>
                                    ...
                                    via-amber-800/50 to-transparent opactity-50 transition-all duration-300' />
                                        ...
                                        <div className='flex itens-center gap-2'>
                                            {qty > 0 ? (
                                                <>
                                                    ...
                                                    justify-center hover:bg-amber-800/50 transitions-colors' onClick={() => qty
                                                            > 1 ? updateQuantity(cartEntry._id, qty - 1) : removeFromCart(cartEntry._id)}>
                                                        <FaMinus className='text-amber-100' />
                                                    </button>
                                                    <span className='w-8 text-center text-amber-100'>
                                                        {qty}
                                                    </span>
                                                    <button className='w-8 h-8 rounded-full bg-amber-900/40 flex items-center justify-center hover:bg-amber-800/50 transitions-colors'
                                                ...
                                                <button onClick={() => addToCart(item, 1)}
                                                    className='bg-amber-900/40 px-4 py-1.5 rounded-full font-cinzel text-xs uppercase
                                                    sm:text-sm tracking-wider transition-transform duration-300 hover:scale-110 hover:shadow-lg hover:shadow-amber-900/20
                                                    relative overflow-hidden border border-amber-800/50'>
                ...
                <div className='flex justify-center mt-16'>
                    ...
                    hover:bg-amber-800/40 hover:text-amber-50 hover:scale-105 hover:shadow-lg hover:shadow-amber-900/20
                    backdrop-blur-sm' to='/menu'>
                        Explore Full Menu
                    </Link>
                </div>
