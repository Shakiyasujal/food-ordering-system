...
const Footer = () => {
    ...
    return (
        <footer className='bg-[#2A211C] text-amber-100 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden'>
            <div className='max-w-7xl mx-auto relatice z-10'>
                <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mb-12'>
                    {/* left column */}
                    <div className='space-y-6'>
                        <h2 className='text-4xl sm:text-5xl md:text-5xl font-bold font-sacremento text-amber-400 animate-pulse'>
                            Mitho-Munch
                        </h2>
                        <p className='text-amber-200/90 text-sm font-scarmento italic'>
                        ...
                        <form onSubmit={handleSubmit} className='relative mt-4 group'>
                            ...
                            <div className='relative'>
                                <input type="email" placeholder='Enter your email...' value={email}
                                    ...
                                    focus:outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-400/20 transition-all duration-300
                                    placeholder-amber-200/50 pr-24' required />
                                <button type='submit' className='absolute right-1 top-1 bg-gradient-to-br from-amber-300
                                via-orange-500 to-amber-600 text-white px-4 py-2 rounded-full flex items-center gap-1.5 shadow-lg
                                hover:shadow-amber-400/30 overflow-hidden transition-all duration-500'>
                                    ...
                                    <BiChevronRight className='text-xl transtion-transform duration-300 group-hover:animate-spin flex-shrink-0' />
                                    <span className='absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-amber-50/30
                                ...
                                {socialIcons.map(({ icon: Icon, link, color, label }, idx) => {
                                    return (
                                        <a target='_blank' href={link} key={idx}
                                            className='text-2xl bg-amber-400/ p-3 rounded-full hover:bg-amber-400/20 hover:scale-110
                ...
                <div className='border-t border-amber-800 pt-8 mt-8 text-center'>
                    <p className='text-amber-400 text-lg mb-2 font-playfair'>
                        &copy; 2025 Mitho-Munch. All rights reserved
                    </p>
                </div>
            </div>
        </footer>
